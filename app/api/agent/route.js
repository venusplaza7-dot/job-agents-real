...
const SENDER_EMAIL = process.env.BREVO_SENDER_EMAIL || "ve9us1@gmail.com";
...
export async function GET() {
  ...
  if (!BREVO_API_KEY) {
    return Response.json({ ...debug, ok: false, error: "BREVO_API_KEY missing in Vercel ENV - Add in Settings > Environment Variables > Production" }, { status: 500 });
  }
  ...
  try {
    ...
    let filtered = jobs.filter((j) => {
      ...
      const isAI = text.includes(" ai ") || text.includes(" ai,") || text.includes("artificial intelligence") || text.includes(" llm") || text.includes(" openai") || text.includes("genai") || (j.tags || []).some((t) => ["ai","llm","machine learning","genai"].includes(t.toLowerCase()));
    ...
    if (filtered.length === 0) {
      filtered = jobs.filter((j) => (j.position || "").toLowerCase().includes("full stack")).slice(0,2);
    }
    ...
    if (SUPABASE_URL && SUPABASE_KEY) {
      try {
        ...
        if (seenRes.ok) {
          ...
          jobsToSend = filtered.filter((j) => !seenIds.has(String(j.id))).slice(0,2);
        }
      } catch {}
    }
    ...
    for (const job of jobsToSend) {
      ...
      const htmlContent = `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;color:#111;line-height:1.6">
          ...
          <p><b>Why me:</b> Full Stack AI (Next.js / Node / Python / LLM / RAG), Brevo/Resend API, Supabase, Vercel Cron, Groq. I ship autonomous systems end-to-end.</p>
          ...
          <p>Best,<br/><b>Ron Kahn</b><br/>Full Stack AI Engineer<br/>Lahore, Pakistan<br/><a href="${RESUME_LINK}">${RESUME_LINK}</a></p>
